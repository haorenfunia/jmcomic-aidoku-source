...
impl Home for JMComic {
	fn get_home(&self) -> Result<HomeLayout> {
		...
		let korean_entries = crate::search_result(
			&api,
		...
		// Keep the most useful entry first so opening the source always starts with
		// the latest Korean webtoons.
		components.insert(
			0,
			HomeComponent {
